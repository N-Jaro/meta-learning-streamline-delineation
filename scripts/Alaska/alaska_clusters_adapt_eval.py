...
def process_adapt_and_evaluate(model_path, csv_folder, data_dir, inner_steps, learning_rate, wandb_project, normalization_type, patience, model_save_path):
    ...
    for csv_file in csv_file_list:
        if csv_file.endswith('_train.csv'):
            ...
            # Adapt the model
            best_adapted_model, best_model_save_path = adapt_model(model, combined_data, combined_labels, inner_steps, learning_rate, save_dir, "Alaska_maml_clusters_adapt", run_name, scenario, patience)

            eval_save_dir =  os.path.join('./clusters_evaluations', run_name)
            # Find the corresponding test file
            test_file = csv_file.replace('_train.csv', '_test.csv')
            if test_file in csv_file_list:
                print(f"Evaluating adapted model on {test_file}")
                
                # Evaluate the adapted model on the test set
                evaluate_model(best_adapted_model, best_model_save_path, test_file, data_dir, normalization_type, eval_save_dir, "Alaska_maml_clusters_eval", run_name, scenario)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='/u/nathanj/meta-learning-streamline-delineation/scripts/Alaska/models/maml_3_500_1_20240909_121616/maml_model.keras', help='Path to the saved .keras model file')
    parser.add_argument('--csv_folder', type=str, default='/u/nathanj/meta-learning-streamline-delineation/scripts/Alaska/data_gen/huc_code_clusters/', help='Folder containing CSV files to process')
    ...
    parser.add_argument('--model_save_path', type=str, default='/u/nathanj/meta-learning-streamline-delineation/scripts/Alaska/models/model_adapt_09272024/', help='Path to save adapted models')
