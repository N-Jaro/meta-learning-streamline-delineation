...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dir', type=str, default="models/", help='Path to the directory containing MAML models')
    ...
    # Iterate over all MAML models in the directory
    model_paths = glob.glob(os.path.join(args.model_dir, 'maml_1_500_1_20240617_*'))
    for model_path in model_paths:
        maml_model_path = os.path.join(model_path, 'maml_model.keras')
        if os.path.exists(maml_model_path):
            # Initialize WandB for this model
            run_name = f"adapt_{os.path.basename(model_path)}_{args.testing_locations[0]}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            config = {
                "model_path": maml_model_path,
            ...
            maml_model = tf.keras.models.load_model(maml_model_path)
            ...
            print(f"Test Loss after adaptation for model {model_path}: {test_loss}")

            # Save the adapted model
            adapted_model_path = os.path.join(model_path, f'adapted_{args.testing_locations[0]}.keras')
            ...
            print(f"Evaluation scores for location {args.testing_locations[0]} with model {model_path}")
