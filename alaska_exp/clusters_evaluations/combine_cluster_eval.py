...
def extract_cluster_number(filename):
    ...
    for part in parts:
        if part.startswith('cluster'):
            return int(part.replace('cluster', '').split('.')[0])
    raise ValueError(f"Cluster number not found in {filename}")
...
num_clusters = "5"  # Can be "5" or any other number
case = "regular"    # Can be "regular" or "random"

# Create the search pattern using the parameters
paths = f"{case}_{num_clusters}clusters_cluster*/{case}_{num_clusters}clusters_cluster*.csv"
