...
def train_model(model, train_dataset, val_dataset, epochs, learning_rate, patience, args, config, cluster_id):
    ...
    wandb.init(project=args.wandb_project, name=f"{args.wandb_run_name}_cluster_{cluster_id}", config=config, group=args.wandb_run_name, job_type="train_process")
...
def prepare_env(args):
    ...
    run_name = f"scratch_training_{args.num_sample_per_huc_code}_samples_{date_time}"
...
def main(args):
    ...
    # Initialize WandB
    wandb.init(project=args.wandb_project, name=args.wandb_run_name, config=config, group=args.wandb_run_name)
    ...
    for train_csv_path in train_csv_files:
        ...
        scenario = extract_scenario_from_filename(train_csv_path)
        ...
        train_data, train_labels = load_data_and_labels_from_csv(train_csv_path, args.data_dir, args.channels, args.num_sample_per_huc_code, args.normalization_type)
        ...
        trained_model, trained_history = train_model(model, train_dataset, val_dataset, args.epochs, args.learning_rate, args.patience, args, config, scenario['cluster_id'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_save_dir', type=str, default='/u/nathanj/meta-learning-streamline-delineation/alaska_exp/train_scratch_exp/wo_254_255/experiments/', help='Directory to save the trained models')
