...
def main(args):
    ...
    # create run_name for the run in the initialization of the MAMLTrainer
    maml_trainer = MAMLTrainer(model, meta_train_episodes, config, args)

    # Initialize train initial model wandb is initialized in the train function
    maml_model, init_model_path = maml_trainer.train()

    # Initialize WandB for adaptation runs
    adapt_cluster_run_name = f"{maml_trainer.get_run_name()}_adapt"
    maml_trainer.initialize_wandb(run_name=adapt_cluster_run_name)
    ...
    for adapt_csv_path in adapt_csv_files:
        ...
        # Adapt the model
        adapted_model, adapted_model_path  = maml_trainer.adapt_cluster_fit(adapt_csv_path, init_model_path, scenario, num_samples_per_location=-1)
        
        #evaluate the model
        test_csv_path = adapt_csv_path.replace('_adapt.csv', '_test.csv')
        test_metrics = maml_trainer.evaluate_clusters(adapted_model, test_csv_path, scenario, threshold=0.33) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb_project_name', type=str, default='test_maml_exp', help='Path to save trained models')
    ...
    parser.add_argument('--num_episodes', type=int, default=15, help='Number of episodes')
